// 상수와 변수
    // 1. const 상수만들기        키워드 작성 왜? 상수인지 변수인지 식별용
    // 2. 상수명            이름[식별자] 왜? 상수가 여러개 있으므로 그 중에서 구별 남 중에서 
    // 3. =                 대입 연산자 [값을 변수에 넣는 역할] 왜? 오른쪽 값을 왼쪽에 대입
    // 4. 자료              저장할 값

const pi = 3.141592;

// 2. 상수 호출하는 방법 [상수 이름 작성]
console.log( pi );  //상수 출력
//console.log( 차준영 ); // 차준영 is not defined

// 3. 상수의 값을 변경하는 방법 [ 호출해서 다시 대입 ] !!! 키워드 x
//pi = 3.97;
// step2.js:11 Uncaught ReferenceError: 차준영 is not defined


const r = 10;

// 4. 상수와 연산
console.log ( 2 * pi * r);
// pi 상수[상자] 안에 있는 값을 꺼내기 우선 = 상수 호출 | 상수의 값을 호출 후 곱하기 

// 5. 동일하 ㄴ상수명 사용 [ 상수명은 식별용 이므로 중복 이 불가능]
//const pi = 3.97;

// 6. 상수의 값 대입을 안했을때.[ 상수 만들때[처음에] 값을 넣는 과정 = 초기화 없으면 오류발생]
//const pi2;

// 변수 만들기 (var,let)
    //1. let
    //2. 변수명
    //3. =
    //4. 값


let pi3 = 3.141592;
//  2. 변수 호출 [ 변수 호출시에는 키워드 [let,const] 쓰지마세요.!!!!]
console.log( pi3 );
console.log( "pi3 : " + pi3 ); //연결연산자 : "문자열" + 변수의 값(숫자) ==> 문자열
console.log( `pi3 : ${ pi3 }` );  //템플릿 문자열 `문자열 ${ 변수/상수/값}~~`

// 3. 변수의 값 호출 변경
pi3 = 3.15; console.log( pi3 );

// 4. 변수의 연산
let r3 = 10;
console.log( 2 * pi3 * r3);

// var와 let 키워드 차이
let value1 = 10;  //불가능 초기화 1번 [동일한 변수명으로 선언시 선언불가.]
//let value1 = 10;

var value2 = 10; //가능 초기화 여러번 [ 동일한 변수명으로 선언시 동일한 이름이 있을경우 덮어쓴다.]
var value2 = 10; 

// 복합 대입 연산자
let value = 10;

        // 컴퓨터는 동시에 다발적으로 연산을 처리x 연산 하나씩 처리
value += 10;
    //VS        같은결과를 얻는 표현방법
value = value + 10; //문장 
                    // = 기준으로오른쪽 연산
                           
                    //1. 10             // value + 10 중에서 변수 호출 호출
                    //2. 10 + 10        //호출 후 연산
                    //3. 20             // 연산 결과
                    //4. =              // 연산 후 대입
                    //5. value = 20     // 변수 수정

console.log( value );

//******* JS에서 HTML(문자열 자료형) 를 작성할때.
let list= ``;

list += `<ul>`;
list += `   <li>Hello</li>`
list += `   <li>JavaScript</li>`
list += `</ul>`;

document.write( list );

//=====================================================================================
// 예제 1 : 2개의 숫자를 입력받아 더한 결과 값을 HTML에 <h3>로 출력
    // 입력받은 예)  3, 5   
    // HTML 출력 예)  예제 1 결과: 8

      // 1.prompt("출력하고싶은거");  :입력[input] 제공하는 함수
            // input 입력된 결과를 반환[ 10+10 => 20 ]
     let ex1Value1 = prompt("예1 숫자를 입력하세요");
     console.log( typeof ex1Value1 );
        // 타입 변환 1 문제점: 혹여나 소수점 있으면 사라짐

     ex1Value1 = parseInt ( ex1Value1 );
     console.log( typeof ex1Value1 );
        // 타입 변환 2 [Number( "숫자") -> 숫자 ]

    let ex1Value2 = Number ( prompt("예1 2번째 숫자 입력하세요.") );
            // 함수1( 함수2() ); //함수 2 결과를 함수 1 넣고 함수 1 결과 얻는 과정

    document.write( `<h3> 예제1 결과 ${ ex1Value1 + ex1Value2 } </h3> ` );


// =========================================================================
    
// 예제 2 : 2개의 숫자를 입력받아 더 큰수를 HTML에 <h3> 안에 출력
    // 출력 예 예제2 결과 : 5

        // 1. 입력받은 데이터를 숫자자료형으로 변환[ 숫자로 연산할려고/문자로 연산할수는 없으니까] 후 각 변수에 대입
    let ex2Value1 = Number( prompt("예제2 1번째 숫자 입력하세요.") );
    let ex2Value2 = Number( prompt("예제2 2번째 숫자를 입력하세요") );
    
    console.log( ex2Value1 > ex2Value2 );

        // 2. 
        // 단항연산자 [ 피연산자 1개] , 이항 연산자[ 피연산자 2개] , 삼항 연산자[피연산자3개]
        // 삼항연산자 : 조건? 참 : 거짓     [:클론 ;세미클론]
            // - 조건[T/F]에 따른 서로 다른 실행 결과 만들기

    console.log( ex2Value1 > ex2Value2 ? '진실' : '거짓' ); //false 대신에 '거짓' 표현

    document.write( `<h3> 예제2 결과 : $ { ex2Value1 > ex2Value2 ? ex2Value1 : ex2Value2 } </h3>` );

//==========================================================================







