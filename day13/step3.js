...
/*
 ...
 for ( 초기값 ; 조건문 ; 증감식 ){ 실행코드; }
    ...
    2. i < 반복횟수      : 반복을 할수 있는 조건[ 조건을 충족시 반복실행]
        i는 반복횟수보다 작으면 
    3. 문장              : 반복시 실행되는 코드
    4. i++               : 1회 반복후 실행되는 증감연산
 
- 실행순서
    1. [1초기값] 변수에 처음값을 초기화.
    2. [2조건문] for{}안/반복문 에 들어갈수 있는지 검사 [t->{}안으로 가능 f->{}밖으로 불가능] 
...
for (let i = 1 ; i <=10 ; i++ ) { console.log('for)우유구매'); }
 // -let i는 1부터 10이하까지 1씩증가 반복하면서 console.log 출력할꺼야. 총 10회
 ...
 for (let i = 0 ; i <5; i ++) { //for start
    // -let i는 0부터 5미만까지 1씩증가하면서 console.log반복실행.0.1.2.3.4 총5회
 ...
 for (let i = 1; i <=100 ; i++){ //for s
    // -let i는 1부터 100이하까지 1씩증가하면서ouput에 i를 더하는 반복실행
    output += i;
    // -output에 i를 더한 값을 output에 저장.
    //console.log( i ); //스타트 부터 끝날때까지를 보여줌
...
for( let i = 0 ; i < todos.length; i++){
    // -i는 0부터 todos배열의 마지막인덱스까지 1씩증가하면서 console.log 반복실행.
    console.log(`${i}번째 할 일 : ${todos[i]}`);
    // i는 인덱스가 아니지만.. i처럼 사용되고 있음. 
    // 인덱스 대신에 i 활용
}

// 예제 3 역순.
for( let i = todos.length -1 ; i >= 0; i--){
    // -i는 마지막인덱스부터 0까지 1씩감소 하면서 console.log 반복 실행.
    console.log(`${i}번째 할 일 : ${todos[i]}`);
}


let outPrint = ``;  //공백이 저장된 변수.
// 예2 ) 1부터 10이하까지 1씩 증가 반복 [ 한줄 표시]; console.log()하나당 줄바꿈 자동.
for( let i = 1 ; i <=10 ; i++ ){
    outPrint += `  ${i}  `;
}
console.log( outPrint );

// 예3) 1부터 10이하까지 1씩 증가반복 [ul 표시]
...
sum = 0;
for(let i = 1 ; i<=100 ; i++){
    if( i % 7 == 0 ){sum += i ;} //만약에 i가 나누기 7를 했을때 나머지가 0이면 i는 7배수.
}
