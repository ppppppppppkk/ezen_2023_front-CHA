...
// 
const 전국영화관 = [
    { 지점코드 : 1, 지점명:'센트럴락', 주소 : '고잔로 108', 영화 : ['서울의봄', '노랑', '싱글인서울','3일의휴가'] },
    { 지점코드 : 2, 지점명:'안산고잔', 주소 : '고잔로 203',영화 : [ '아쿠아맨','노랑'] },
    { 지점코드 : 3, 지점명:'안산롯데시나마', 주소 : '상록구755', 영화 : [ '싱글인서울'] }

];

전국영화관출력구역();           //함수 호출
function 전국영화관출력구역(){  //함수 선언
    //1. 어디에
    ...
    let html =""
        ...
        for ( let i = 0 ; i<전국영화관.length; i++){
            html += `<span style="margin:10px" onclick="영화출력( ${ 전국영화관[i].지점코드} )"> 
            ${전국영화관[i].지점명} </span>`
        }
    // 3. 출력
...
function 영화출력( 선택한지점코드 ){ //함수 선언 [ 실행조건 : 영화관을 이름을 클릭했을때]
